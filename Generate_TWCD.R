...
# Load in data
setwd("E:/ftms/长期施肥数据/36个样") 
mol = read.csv(list.files(pattern = "rhizo_mol.csv"), row.names = 1) # Load in the molecular data
peak.2.peak = read.csv(list.files(pattern = "rhizo_All-Trans_peak.2.peak.csv")) # Load in the peak.2.peak file for all/bulk transformations
num.trans = read.csv(list.files(pattern = "rhizo_All-Trans_num.peak.trans.csv")) # Load in the num.peak.trans file for all/bulk transformations
...
Mol.Info = mol[,c("C", "H", "O", "N", "DBE", "AI", "KMD"), drop = F]
Mol.Ratio = mol[,c("O_C", "H_C", "N_C")]
...
# Writing the tree
write.tree(tree, paste(Sample_Name, "tree_TWCD.tre", sep = ""))
