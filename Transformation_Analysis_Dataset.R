...
# Loading in ICR data
setwd("E:/ftms/长期施肥数据/36个样/每个样的转化")
...
data = read.csv(list.files(pattern = "S36.csv"), row.names = 1) # Keeping data and mol-data seperate to ensure they are unaltered
mol = read.csv(list.files(pattern = "clean_mol3.csv"), row.names = 1)
...
trans.full =  read.csv("database.csv")
...
colnames(bulk.peaks) = c("peak", "Sample_S36")
...
for(i in start_peak:(nrow(data)-1)){ # I cannot stress the importance of the "-1" here...
  ...
  if(!dir.exists(paste("Bulk Peak Transformations (do not backup)", Sample_Name, sep = ""))){
    dir.create(paste("Bulk Peak Transformations (do not backup)", Sample_Name, sep = ""))
  }
  
  write.csv(Distance_Results, paste0("Bulk Peak Transformations (do not backup)", Sample_Name, "/Distance_Results_", i, "_for_peak_", row.names(data)[i], ".csv"),
            quote = F, row.names = F)
...
write.csv(peak.2.peak, paste(Sample_Name, "_Trans.csv", sep = ""), quote = F, row.names = F)
write.csv(peak.profile, paste(Sample_Name, "_Trans of each peak.csv", sep = ""), quote = F, row.names = F)


trans <- read.csv("S36_trans.csv", header = T)
eachtrans <- read.csv("S36_Trans of each peak.csv", header = T)
summary(trans$sample)
summary(eachtrans$num.trans.involved.in)
