...
Sample_Name = "S1" # This is a name that will be added to the output file (helps in tracking)
...
# Loading in ICR data
setwd("E:/2019连续标记/高分辨质谱结果/nti")
data = read.csv(list.files(pattern = "S1.csv"), row.names = 1) # Keeping data and mol-data seperate to ensure they are unaltered
mol = read.csv(list.files(pattern = "rhizo_mol.csv"), row.names = 1)
...
str(tot.trans)
write.csv(tot.trans,paste(Sample_Name,"S1_Total_Transformations.csv", sep=""),quote = F,row.names = F)
